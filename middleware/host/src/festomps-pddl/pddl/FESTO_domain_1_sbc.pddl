...
(define (domain FESTO_domain)
	...
	(:predicates 
	     ...
	     (drilled ?obj - unit)
             (faceup ?obj - unit)
             (color-sensor-on ?pos - (either robotposition beltposition plateposition))
	...
	(:action rod-push
		...
		:effect (and  (at ?obj ?to) (not (at ?obj ?from)) (occupied ?to) (not (occupied ?from))
			)
	)	
	...
	(:action trigger-color-sensor
		:parameters  (?obj - unit ?pos - robotposition)
		:precondition  (and (P0TRAN) (at ?obj ?pos) (in-robot ?pos) )
		:effect (and (color-sensor-on ?pos) (not (P0TRAN))
			)
	)		
	(:action return-color-value
		:parameters  (?obj - unit ?pos - robotposition ?color - colortype)
		:precondition  (and (not (P0TRAN)) (color-sensor-on ?pos) (in-robot ?pos) (at ?obj ?pos))
		:effect (and (not (color-sensor-on ?pos)) (P0TRAN) (color ?obj ?color)
			)
	)
